...
**明显简化了整体流程，基本实现一键化并提高可视化程度**（本程序为测试版程序，如有 bug 反馈可按下方联系方式提交）。
...
![登录与用户选择](docs/screenshots/01-welcome.jpg)

**进入程序先选用户与医院**：输入姓名、选好医院即可开始。
**24 级规培轮转科室已自动依照官方 Excel 收录**，输入姓名 + 医院即可自动匹配你的轮转科室；
其他同学如有需要，可在**设置界面**修改对应的 Excel，或把表格提交过来收录。
